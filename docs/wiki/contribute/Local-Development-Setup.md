...
To work on `csp-gateway`, you are going to need to build it from source. See
[Build from Source](Build-from-Source) for
detailed build instructions.

Once you've built `csp-gateway` from a `git` clone, you will also need to
configure `git` and your GitHub account for `csp-gateway` development.

## Step 2: Configuring Git and GitHub for Development

### Create your fork

The first step is to create a personal fork of `csp-gateway`. To do so, click
the "fork" button at https://github.com/Point72/csp-gateway, or just navigate
[here](https://github.com/Point72/csp-gateway/fork) in your browser. Set the
